...
export interface TerminalPanelHandle {
  ...
  openTerminal: () => void;
  setActiveTab: (tab: "output" | "terminal" | "problems") => void;
  clear: () => void;
  reconnect: () => void;
  restart: () => void;
  focus: () => void;
...
export const TerminalPanel = forwardRef<TerminalPanelHandle, TerminalPanelProps>(
  function TerminalPanel(
    ...
    // Forward imperative methods to parent (e.g. IDE layout Run button)
    useImperativeHandle(
      ref,
      () => ({
        ...
        syncFile: (filePath: string, content: string) => {
          xtermRef.current?.syncFile(filePath, content);
        },
        openTerminal: () => {
          if (!isOpen) onToggle();
        ...
        isTerminalConnected: () => terminalStatus === "connected",
      }),
      [isOpen, onToggle, terminalStatus]
    );
        ...
        {!isOpen ? (
          <div className="flex h-8 w-full items-center justify-between border-b border-border/20 bg-muted/40 px-3 select-none text-xs">
            ...
            >
              ...
              <span>DevForge Interactive Shell</span>
              <Circle
                className={cn(
          ...
          <div className="flex h-8 items-center justify-between border-b border-border/40 bg-muted/30 px-3 text-xs">
            ...
            {/* Actions: Reconnect, Clear, Expand, Collapse */}
            <div className="flex items-center gap-1">
              {/* Detected Server button if running dev server */}
              {detectedServers.length > 0 && (
                <button
                  type="button"
                  onClick={() => onOpenPreview?.(detectedServers[0].url)}
                  className="flex items-center gap-1 text-[11px] font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded cursor-pointer hover:bg-emerald-500/20 mr-1 animate-pulse"
                  title="Open detected application in Live Preview"
                >
                  <Globe className="size-3" />
                  <span>Port :{detectedServers[0].port} Active</span>
                </button>
              )}
              ...
              {activeTab === "terminal" && (
                <>
                  <Button
                    ...
                    onClick={() => xtermRef.current?.restart()}
                  >
              ...
              <Button
                ...
                size="icon"
                className="size-6 text-muted-foreground hover:text-foreground cursor-pointer"
                title={isExpandedFull ? "Restore Height" : "Maximize Panel"}
              ...
              <div className="space-y-1">
                {logs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-6 text-muted-foreground/60 select-none">
                    ...
                    <p>No output yet. Click &quot;Run&quot; above to execute current file in terminal.</p>
                  </div>
                ) : (
                  logs.map((log, index) => {
